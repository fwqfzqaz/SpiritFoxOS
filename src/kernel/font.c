...
static const uint8_t font_data[128][16] = {
    ...
    /* 0x1C */ { 0x00, 0x00, 0x00, 0x00, 0x00, 0xC0, 0xC0, 0xC0, 0xC0, 0xFE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
    ...
    /* 0x4C 'L' */ { 0x00, 0x00, 0xF0, 0x60, 0x60, 0x60, 0x60, 0x60, 0x60, 0x60, 0x62, 0x66, 0xFE, 0x00, 0x00, 0x00 },
    ...
    /* 0x56 'V' */ { 0x00, 0x00, 0xC6, 0xC6, 0xC6, 0xC6, 0xC6, 0xC6, 0x6C, 0x38, 0x38, 0x10, 0x00, 0x00, 0x00, 0x00 },
...
void font_draw_char(uint32_t x, uint32_t y, char c, uint32_t fg, uint32_t bg)
{
    ...
    uint32_t *buf = font_buf();
    uint32_t stride = font_stride();
    uint32_t *row_base = buf + y * stride + x;
    ...
    for (uint32_t row = 0; row < FONT_HEIGHT; row++) {
        ...
        if (glyph & 0x10) pixel[3] = fg; else pixel[3] = bg;
        if (glyph & 0x08) pixel[4] = fg; else pixel[4] = bg;
...
void font_draw_string(uint32_t x, uint32_t y, const char *str, uint32_t fg, uint32_t bg)
{
    ...
    while (*str) {
        ...
        uint32_t *row_base = buf + y * stride + cx;
        for (uint32_t row = 0; row < FONT_HEIGHT; row++) {
            uint8_t glyph = font_data[ch][row];
            uint32_t *pixel = row_base;
            if (glyph & 0x80) pixel[0] = fg; else pixel[0] = bg;
...
void font_draw_string_transparent(uint32_t x, uint32_t y, const char *str, uint32_t fg)
{
    ...
    while (*str) {
        ...
        for (uint32_t row = 0; row < FONT_HEIGHT; row++) {
            ...
            if (glyph & 0x10) pixel[3] = fg;
            if (glyph & 0x08) pixel[4] = fg;
            if (glyph & 0x04) pixel[5] = fg;
            if (glyph & 0x02) pixel[6] = fg;
            if (glyph & 0x01) pixel[7] = fg;
            row_base += stride;
        }
        cx += FONT_WIDTH;
        str++;
